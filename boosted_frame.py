...
#      fs.aftersetsourcep(lzero=1)
  ...
  def boosted_output_method(self):
    ...
    ##converted time when the snapshot is taken
    ...
    if self.l_beam == 1:
      ...
      #zboost_mid = map(lambda x: (zboost_prev[x]+zboost[x])/2,ix)
      ...
      # find particles that crossed the output plane
      ii = map((lambda x: numpy.compress(numpy.logical_or(numpy.logical_and(numpy.less_equal(zboost[x],zb),numpy.less_equal(z_prevb,zboost_prev[x])),numpy.logical_and(numpy.less_equal(zb,zboost[x]),numpy.less_equal(zboost_prev[x],z_prevb))),iib)), ix)
      ...
      ux_filtered       = numpy.array(map(lambda x: uxb[ii[x]], ia)) 
      uy_filtered       = numpy.array(map(lambda x: uyb[ii[x]], ia))
      uz_filtered       = numpy.array(map(lambda x: uzb[ii[x]], ia))
      ...
      if True:
				...
				##Interpolation to zboost

				x			= numpy.array(map(lambda xx : xo[xx]*(z[xx]-zboost[xx])/(z[xx]-zo[xx])+x[xx]*(zboost[xx]-zo[xx])/(z[xx]-zo[xx]),ia))
				y			= numpy.array(map(lambda xx : yo[xx]*(z[xx]-zboost[xx])/(z[xx]-zo[xx])+y[xx]*(zboost[xx]-zo[xx])/(z[xx]-zo[xx]),ia))
				ux			= numpy.array(map(lambda xx : uxo[xx]*(z[xx]-zboost[xx])/(z[xx]-zo[xx])+ux_filtered[xx]*(zboost[xx]-zo[xx])/(z[xx]-zo[xx]),ia))
				uy			= numpy.array(map(lambda xx : uyo[xx]*(z[xx]-zboost[xx])/(z[xx]-zo[xx])+uy_filtered[xx]*(zboost[xx]-zo[xx])/(z[xx]-zo[xx]),ia))
				uz			= numpy.array(map(lambda xx : uzo[xx]*(z[xx]-zboost[xx])/(z[xx]-zo[xx])+uz_filtered[xx]*(zboost[xx]-zo[xx])/(z[xx]-zo[xx]),ia))
				gammainv= numpy.array(map(lambda xx : gammainvo[xx]*(z[xx]-zboost[xx])/(z[xx]-zo[xx])+gammainv[xx]*(zboost[xx]-zo[xx])/(z[xx]-zo[xx]),ia))

				#print "gammainvaft", gammainv
				##z has to be written after all other parameters
				z			= numpy.array(map(lambda xx : zo[xx]*(z[xx]-zboost[xx])/(z[xx]-zo[xx])+z[xx]*(zboost[xx]-zo[xx])/(z[xx]-zo[xx]),ia))
